...
// put focus on input area  whenever chatbox is opened
const input = ref();
watch(open, () => {
...
watch(
  ...
  () => {
    nextTick(
      // nextTick(): allows you to execute code after you have changed some data and Vue.js has updated the virtual DOM 
      // based on your data change, but before the browser has rendered that change on the page. in this case-> after
      // new message is submitted, wait unit DOM is rendered then scroll it one message box up.
...
<template>
  <div class="fixed bottom-[10px] right-[10px]">
    <!-- Open Chat nbutton. in lower right of screen. @click -> open the chat box.  hide button if open is true -->
    <!-- the button is a component named IconChat -->
    <button v-show="!open" @click="open = true" class="bg-blue-500 p-3 rounded">
      <IconChat class="h-8 w-8 text-white" />
    </button>
    <!-- Div that holds chatbox. use v-if="open" to show the div -->
    <div
      v-if="open"
    ...
    >
      ...
      >
        ...
        <button class="p-4 pr-0" @click="open = false">
          <!-- Down arrow to close chatbox. @click set "open" = false [hide box div] -->
          <svg
